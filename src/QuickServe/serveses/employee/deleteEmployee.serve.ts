...
import { deleteOneEmployeeRequestPreset } from "@SRC/QuickServe/presets/employee.preset";
...
export const DeleteOneEmployee = async (req: Request, res: Response): Promise<any> => {
    try {
        console.log('DeleteOneEmployee : ', req.params);

        const empId = req.params.empId;
        const preset = deleteOneEmployeeRequestPreset(empId);
        const response = await StoreService(preset, 'delete');

        return res.status(200).json({
            message: "Successfully DeleteOneEmployee Served!",
            quick_serve_name: 'DeleteOneEmployee',
            success: true,
            data: {
        ...
        console.log('DeleteOneEmployee (Error):', error);

        if (error?.kind) {
            await errorHandles(error, res, { systemName: 'QuickServe', feature: 'DeleteOneEmployee' });
        } else {
            HandleError(res, error, 'DeleteOneEmployee');
        }
    }
};
