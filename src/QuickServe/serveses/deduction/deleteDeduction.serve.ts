...
import { deleteOneDeductionRequestPreset } from "@SRC/QuickServe/presets/deduction.preset";
...
export const DeleteOneDeduction = async (req: Request, res: Response): Promise<any> => {
    try {
        console.log('DeleteOneDeduction : ', req.params);

        const dedId = req.params.dedId;
        const preset = deleteOneDeductionRequestPreset(dedId);
        const response = await StoreService(preset, 'delete');

        return res.status(200).json({
            message: "Successfully DeleteOneDeduction Served!",
            quick_serve_name: 'DeleteOneDeduction',
            success: true,
            data: {
        ...
        console.log('DeleteOneDeduction (Error):', error);

        if (error?.kind) {
            await errorHandles(error, res, { systemName: 'QuickServe', feature: 'DeleteOneDeduction' });
        } else {
            HandleError(res, error, 'DeleteOneDeduction');
        }
    }
};
