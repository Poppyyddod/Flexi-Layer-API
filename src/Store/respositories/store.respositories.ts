...
import { SqlStoreRespoMethods, NoSqlStoreRespoMethods } from './features/methods';
...
const dbTypeRespository: any = {
    ...
    nosql: async (serviceData: any, theFeature: RespositoryKeys) => {
        try {
            console.log('StoreRespository (dbTypeRespository)(NoSQL):', serviceData, theFeature);

            const respository = NoSqlStoreRespoMethods[theFeature];
            const dataFromTheRespository = await respository(NoSqlHelperFunction)({ ...serviceData });
            // console.log('StoreRespository (dataFromTheRespository)(NoSQL) : ', dataFromTheRespository);

            return dataFromTheRespository;
        } catch (error) {
            console.log('StoreRespository (dbTypeRespository)(NoSQL)(Error):', error);
            throw error;
        }
    }
}
